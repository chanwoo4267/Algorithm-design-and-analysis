...
void Read_Graph_adj_list(int Vnum, vertex* V, int Enum, edge* E) {
	...
	//after reading vertex, edge numbers
	int vf, vr, cost;
...
int BFS_Tree_adj_list(
	...
	while (q.empty() != true)
	{
		...
		//V[cur].flag = true;
		SLL* p = V[cur].S.top();
		//find adjacent unvisited vertex
		while (p != NULL)
		{
			if (V[E[p->i].vf].flag != true) {
				V[E[p->i].vf].flag = true;
				E[p->i].flag = true;
				q.push(V[E[p->i].vf].name);
				cost += E[p->i].cost;
			}
			if (V[E[p->i].vr].flag != true) {
				V[E[p->i].vr].flag = true;
				E[p->i].flag = true;
				q.push(V[E[p->i].vr].name);
				cost += E[p->i].cost;
			}
			p = p->p;
		}
